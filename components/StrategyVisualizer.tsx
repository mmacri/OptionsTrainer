...
export const StrategyVisualizer: React.FC<StrategyVisualizerProps> = ({
  ...
  const detailedLegs = legs.map((leg) => {
    ...
    return {
      ...leg,
      strike: isOption ? optionsData.strikePrice : leg.strike,
      premium: isOption ? optionsData.premium : leg.premium,
    };
  });

  const netPremium = detailedLegs.reduce((sum, leg) => {
  ...
  return (
    <div className="space-y-2">
      {detailedLegs.map((leg, idx) => (
        <div
          key={idx}
          className={`flex items-center gap-2 border px-2 py-1 rounded text-sm ${getActionColor(leg.action)}`}
        >
          {getTypeIcon(leg.type)}
          <span>{`${leg.action} ${leg.type}`}</span>
          {leg.strike !== undefined && (
            <Badge variant="outline">Strike: {leg.strike}</Badge>
          )}
          {leg.premium !== undefined && (
            <Badge variant="outline">Prem: {leg.premium}</Badge>
          )}
        </div>
      ))}
